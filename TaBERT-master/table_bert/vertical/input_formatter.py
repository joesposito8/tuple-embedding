...
class VerticalAttentionTableBertInputFormatter(VanillaTableBertInputFormatter):
    ...
    def get_ngram_overlap_ratio(self, context: List[str], table, ngram_num: int):
        tokens = list(ngrams(context, ngram_num))
        ret = pd.Series([0 for row in table])
        trigrams_series = pd.Series([list(ngrams(row, ngram_num)) for row in table])
    ...
    def get_content_snapshot(self, context: List[str], table: Table):
        table_rows = []

        for row in table.data:
            table_rows.append([element for column in row for element in column])
        table_rows = pd.Series(table_rows)

        cso = self.get_ngram_overlap_ratio(context, table_rows, 1) + self.get_ngram_overlap_ratio(context, table_rows, 3)

        return [table.data[index] for index in list(cso.nlargest(500).index)]

    def get_input(self, context: List[str], table: Table):
        row_instances = []

        table_data = self.get_content_snapshot(context, table)
