...
class VerticalAttentionTableBert(VanillaTableBert):
    ...
    def forward(
        ...
        **kwargs
        ...
        context_encoding, schema_encoding, vertical_table_encoding = self.vertical_transform(
            context_encoding, context_token_mask, table_encoding, table_mask)
        #schema_encoding = torch.squeeze(table_encoding).mean(axis=0)
        #context_encoding = torch.squeeze(context_encoding).mean(axis=0)
        ...
        if masked_column_token_labels is not None:
            ...
            try:
                return context_encoding, schema_encoding, table_encoding, vertical_table_encoding
            except:
                return context_encoding, schema_encoding, table_encoding
    ...
    def encode(
        ...
        try:
            context_encoding, schema_encoding, table_encoding, vertical_table_encoding = self.forward(**tensor_dict)
        except:
            context_encoding, schema_encoding, table_encoding = self.forward(**tensor_dict)
        ...
        try:
            info = {
                ...
                'instances': instances,
                'table_encoding': torch.squeeze(table_encoding),
                'vertical_table_encoding': torch.squeeze(vertical_table_encoding)
            }
        except:
            info = {
                'tensor_dict': tensor_dict,
